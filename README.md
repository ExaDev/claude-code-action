...
**Interactive mode's Bash grant is unscoped.** Unlike every other mode, interactive grants the bare `Bash` tool rather than an enumerated `Bash(<cmd>:*)` list, so it can run any shell command, not just the specific prefixes named in `action.yml`. This is a deliberate trade against the Claude Agent SDK's own permission model: a scoped `Bash(<cmd>:*)` entry only auto-approves a command that matches it exactly, and the SDK decomposes a compound command (`&&`, pipes, `$(...)`, `case` statements) into its individual operations, requiring every one of them to match an allow rule — so a single unlisted step anywhere in an otherwise-ordinary one-liner (a plain `find`, `wc`, or `echo` used to survey a repository, say) blocks the whole command, with no human present in a headless run to approve it. What still bounds this: every disallowed pattern in `action.yml`'s `interactive` case is a scoped deny rule, and deny rules are evaluated before allow rules and are never overridden by a bare allow (see the [Claude Agent SDK's permission docs](https://code.claude.com/docs/en/agent-sdk/permissions)), so `gh pr merge`, `gh pr close`, `gh issue close`, `gh release`, `gh secret`, `gh variable`, `gh workflow`, `gh repo delete`, `git reset`, `git rebase`, and `git tag` are still blocked regardless of the broader grant. The token this run authenticates with is also independently capped at `contents`/`pull_requests`/`issues` write (`setupGitHubToken`'s `DEFAULT_PERMISSIONS`, since interactive sets no `ADDITIONAL_PERMISSIONS`), so broader shell visibility does not widen what the GitHub API itself will accept from it. What genuinely is new: interactive can now run any command within that token's reach — `curl`, arbitrary scripts, reading environment variables — not just the build/test/git/gh prefixes previously enumerated. That is a real widening of what "assume the prompt can be subverted and check the blast radius" needs to account for in this specific mode; it is not covered by the token-scope or branch-protection controls described elsewhere in this section.
